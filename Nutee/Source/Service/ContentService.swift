...
struct ContentService {
    ...
    // MARK: - Post
    func postDelete(_ postId: Int, completion: @escaping (NetworkResult<Any>) -> Void) {
        
        let URL = APIConstants.PostDelete + "/\(postId)"
        let headers: HTTPHeaders = [
            "Content-Type": "application/json",
            "Cookie" : KeychainWrapper.standard.string(forKey: "Cookie")!
        ]
        ...
        Alamofire.request(URL, method: .delete, encoding: JSONEncoding.default, headers: headers).responseData{
            ...
            case .success:
                ...
                if let status = response.response?.statusCode {
                    switch status {
                    case 200:
                        completion(.success("postDelete에 성공했습니다."))
                    case 401:
                        print("실패 401")
                        completion(.pathErr)
                    case 500:
    ...
    func getHashPosts(_ hashTag: String,_ postCnt: Int, lastId: Int, completion: @escaping (NetworkResult<Any>) -> Void){
       
        let encodingText = hashTag.stringByAddingPercentEncodingForFormData(plusForSpace: true)!

        let URL = APIConstants.Hashtag + encodingText + "?lastId=" + "\(lastId)" + "&limit=" + "\(postCnt)"
        let header: HTTPHeaders = [
            "Content-Type" : "application/json",
            "Cookie" : KeychainWrapper.standard.string(forKey: "Cookie") ?? ""
        ]
